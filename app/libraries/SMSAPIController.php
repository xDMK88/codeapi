...
/*define("SMSC_LOGIN", "AlexAb1980");			// логин клиента
define("SMSC_PASSWORD", "gs89HJ1scvb854M");	// пароль
...
{
    const SMSC_LOGIN='AlexAb1980';
    const SMSC_PASSWORD='gs89HJ1scvb854M';
    const SMSC_POST='0';
    const SMSC_HTTPS='0';
...
// либо массив (<id>, -<код ошибки>) в случае ошибки
    ...
    public function send_sms($phones, $message, $translit = 0, $time = 0, $id = 0, $format = 0, $sender = false, $query = "", $files = array())
    {
        ...
        $m = SMSAPIController::_smsc_send_cmd("send", "cost=3&phones=".urlencode($phones)."&mes=".urlencode($message).
        ...
        if (SMSAPIController::SMSC_DEBUG) {
            if ($m[1] > 0)
    ...
    {
        return mail("send@send.smsc.ru", "", SMSAPIController::SMSC_LOGIN.":".SMSAPIController::SMSC_PASSWORD.":$id:$time:$translit,$format,$sender:$phones:$message", "From: ".SMSAPIController::SMTP_FROM."\nContent-Type: text/plain; charset=".SMSAPIController::SMSC_CHARSET."\n");
    }
...
// возвращает массив (<стоимость>, <количество sms>) либо массив (0, -<код ошибки>) в случае ошибки
    ...
    public function get_sms_cost($phones, $message, $translit = 0, $format = 0, $sender = false, $query = "")
    {
        ...
        $m = SMSAPIController::_smsc_send_cmd("send", "cost=1&phones=".urlencode($phones)."&mes=".urlencode($message).
            ($sender === false ? "" : "&sender=".urlencode($sender)).
...
// либо массив (0, -<код ошибки>) в случае ошибки
    ...
    {
        $m = SMSAPIController::_smsc_send_cmd("status", "phone=".urlencode($phone)."&id=".urlencode($id)."&all=".(int)$all);
...
// возвращает баланс в виде строки или false в случае ошибки
    ...
    {
        $m = SMSAPIController::_smsc_send_cmd("balance"); // (balance) или (0, -error)
...
// Функция вызова запроса. Формирует URL и делает 5 попыток чтения через разные подключения к сервису
    ...
    {
       //              (SMSC_HTTPS ? "https" : "http")."://smsc.ru/sys/$cmd.php?login=".urlencode(SMSC_LOGIN)."&psw=".urlencode(SMSC_PASSWORD)."&fmt=1&charset=".SMSC_CHARSET."&".$arg;
        $url = $_url = (SMSAPIController::SMSC_HTTPS ? "https" : "http")."://smsc.ru/sys/$cmd.php?login=".urlencode(SMSAPIController::SMSC_LOGIN)."&psw=".urlencode(SMSAPIController::SMSC_PASSWORD)."&fmt=1&charset=".SMSAPIController::SMSC_CHARSET."&".$arg;
        ...
        do {
            ...
            $ret = SMSAPIController::_smsc_read_url($url, $files, 3 + $i);
        }
        while ($ret == "" && $i < 5);
...
// curl или fsockopen (только http) или включена опция allow_url_fopen для file_get_contents
    ...
    {
        $ret = "";
        $post = SMSAPIController::SMSC_POST || strlen($url) > 2000 || $files;
        ...
        elseif ($files) {
            ...
            if (!SMSAPIController::SMSC_HTTPS && function_exists("fsockopen"))
            {
                $m = parse_url($url);
